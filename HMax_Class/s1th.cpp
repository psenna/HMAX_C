...
{
#ifdef CUDAON2
    stream.enqueueUpload(image, this->image);
#else
    this->image = image;
#endif
    this->sizes = sizes;
...
void S1Th::roda(){
...
#ifdef CUDAON2
        cv::gpu::resize(image, image, cv::Size(MENORTAMIMG, novaRow), 0, 0, cv::INTER_CUBIC, this->stream);
#else
...
#ifdef CUDAON2
        cv::gpu::resize(image, image, cv::Size(novaCol, MENORTAMIMG), 0, 0, cv::INTER_CUBIC, this->stream);
#else
    ...
    for(int i = 0; i < NUMBANDAS; i++){
        ...
        for(int j = 0; j < (int)orientation->size(); j++){
            ...
            cv::gpu::filter2D(image, result->imgFiltrada[j], CV_8U, *filter, cv::Point(-1,-1), cv::BORDER_DEFAULT, this->stream);
#else
        ...
        result++;
#ifdef CUDAON2
        cv::gpu::resize(image, image, cv::Size(0, 0), REDUCAOS1, REDUCAOS1, cv::INTER_CUBIC, this->stream);
#else
